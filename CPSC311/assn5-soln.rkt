                   ...
                   (local [(define-values (the-fun f-sto) 
                     ...
                     (type-case Value the-fun
                       [closureV (arg-names body closure-env)
                                 ...
                                 (when (not (= (length arg-names) (length arg-vals)))
     ...
     (local [(define (parse exp)
       ...
       (match sexp
         ...
         [(list 'new 'ex-nihilo)
          ...
          (parse-qs `{fun {msg}
                          {ifdef msg           ; does this object have a field named msg?
                                 ...
                                 {error {append "object does not have property " msg}}}})]
         ...
         [(list 'new (list 'prototype p))
          ; This behaves very much like the {new ex-nihilo} case above. HOWEVER, 
          ; If the object has a prototype, it still looks much like the 
          ; This is the "lookup" function for fields in the object.
          ;
         ...
         ;       it's worth understanding this as well. :)
...
; DONE #9: For 1 bonus point (which is ALL the private tests are for): Design a
; QuipuScript expression that can be filled into this program at the ellipsis (the "..."):
;
;  {with {{o ...}}
;    {-> o factorial 10}}
;
